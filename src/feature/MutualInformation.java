...
public class MutualInformation implements IFeatureSelector {
    @Override
    public TreeSet<String> Filter(ICategory category, CategoriesSet catSet, int k){
        ...
        for(String s:catSet.getVocabulary())
        {
            if(category.GetDocumentCountPerTerm(s) > 0)
            {
                ...
                double I = N11/N * Math.log(N*N11/((N10+N11)*(N01+N11))) +
                        N01/N * Math.log(N*N01/((N00+N01)*(N01+N11))) +
                        N10/N * Math.log(N*N10/((N10+N11)*(N10+N00))) +
